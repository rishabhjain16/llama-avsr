...
### Auto-AVSR Integration 🔗

This repository now supports using pretrained **Auto-AVSR** models as video encoders. Auto-AVSR is a state-of-the-art visual speech recognition model that can be used as a feature extractor for the video modality. To use Auto-AVSR:

1. Download a pretrained Auto-AVSR model from the [Auto-AVSR model zoo](https://github.com/mpc001/auto_avsr#model-zoo)
2. Set `--video-encoder-name auto-avsr` and `--pretrain-auto-avsr-path path_to_autoavsr_model.pth`
3. The Auto-AVSR encoder will extract 512-dimensional video features that are then projected to the LLM token space
...
- `video-encoder-name`: The pre-trained video encoder. Choices: [`av-hubert`, `auto-avsr`]. Default: `av-hubert`.
- `pretrain-auto-avsr-path`: Path to pretrained Auto-AVSR model checkpoint (required when using `auto-avsr` video encoder).
...
You can also use a pretrained Auto-AVSR model as the video encoder:

```Shell
python train.py --exp-dir path_to_exp_dir --root-dir path_to_root_dir --project-wandb wandb_project_name \
--exp-name VSR_autoavsr_exp --modality video --llm-model meta-llama/Llama-2-7b-hf \
--video-encoder-name auto-avsr --pretrain-auto-avsr-path path_to_autoavsr_ckpt \
--add_PETF_LLM lora --unfrozen_modules peft_llm --reduction_lora 64 \
--alpha 8 --downsample-ratio-video 3 --num-nodes 1 --gpus 8 --max-frames-video 1000 --lr 5e-4 
```
...
**Example 1b - Using Auto-AVSR as Video Encoder:**

For `AVSR` with `Auto-AVSR` as video encoder and `Whisper Medium` as audio encoder:

```Shell
python train.py --exp-dir path_to_exp_dir --root-dir path_to_root_dir --project-wandb wandb_project_name \
--exp-name AVSR_autoavsr_exp --modality audiovisual --audio-encoder-name openai/whisper-medium.en \
--video-encoder-name auto-avsr --pretrain-auto-avsr-path path_to_autoavsr_ckpt \
--llm-model meta-llama/Meta-Llama-3.1-8B --unfrozen_modules peft_llm \
--add_PETF_LLM lora --reduction_lora 64 --alpha 8 --downsample-ratio-audio 3 --downsample-ratio-video 3 \
--num-nodes 1 --gpus 8 --max-frames-audiovisual 1000 --lr 1e-3
```
...
### For Video-Only Speech Recognition (VSR):
```bash
python train.py \
    --exp-dir ./experiments \
    --root-dir /path/to/your/lrs3/dataset \
    --project-wandb your_wandb_project \
    --exp-name VSR_AutoAVSR_Llama31 \
    --modality video \
    --llm-model meta-llama/Meta-Llama-3.1-8B \
    --video-encoder-name auto-avsr \
    --pretrain-auto-avsr-path /home/rishabh/Desktop/Experiments/Llama-AVSR/ckps/vsr_trlrs3vox2_base.pth \
    --add_PETF_LLM lora \
...
### For Audio-Visual Speech Recognition (AVSR):
```bash
python train.py \
    --exp-dir ./experiments \
    --root-dir /path/to/your/lrs3/dataset \
    --project-wandb your_wandb_project \
    --exp-name AVSR_AutoAVSR_Whisper_Llama31 \
